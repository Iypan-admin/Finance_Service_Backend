...
const FONT_PATHS = {
    ...
    montserrat: path.join(__dirname, "..", "fonts", "montserrat-semibold.ttf"), // fixed
};

// --- Ensure fonts exist ---
for (const [key, fontPath] of Object.entries(FONT_PATHS)) {
...
function makeTextSVG({ width, height, cardKey, name, cardNumber, validFrom, validThru }) {
    ...
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      <style>
        @font-face { font-family: 'Poppins'; src: url('data:font/ttf;base64,${FONT_BASE64.poppins}') format('truetype'); }
        @font-face { font-family: 'Orbitron'; src: url('data:font/ttf;base64,${FONT_BASE64.orbitron}') format('truetype'); }
        @font-face { font-family: 'Montserrat'; src: url('data:font/ttf;base64,${FONT_BASE64.montserrat}') format('truetype'); }

        .name { font-family: 'Poppins'; font-weight: 800; font-size: ${ns}px; fill: #ffffff; letter-spacing: 1px; }
        .number { font-family: 'Orbitron'; font-weight: 900; font-size: ${cs}px; fill: #ffffff; letter-spacing: ${tracking + 1}px; }
        .valid { font-family: 'Montserrat'; font-weight: 600; font-size: ${fs}px; fill: #ffffff; letter-spacing: 0.5px; }
      </style>
...
async function renderFront(cardName, { name, cardNumber, validFrom, validThru }) {
    ...
    const outBuf = await img.composite([{ input: svg, top: 0, left: 0 }]).jpeg({ quality: 95 }).toBuffer();
    ...
    return await new Promise((resolve, reject) => {
        ...
        sharp(frontJpegBuffer)
            ...
            .then(({ width, height }) => {
                ...
                return sharp(backBuf)
                    .metadata()
                    .then(({ width: bw, height: bh }) => {
...
// --- Generate + Upload PDF ---
async function generateAndUploadCardPDF({ cardName, name, cardNumber, validFrom, validThru, bucket = "elite-cards", pathPrefix = "cards/" }) {
    if (!cardName) throw new Error("cardName is required");
    if (!cardNumber) throw new Error("cardNumber is required");

    const { buffer: frontBuf, backPath } = await renderFront(cardName, { name, cardNumber, validFrom, validThru });
    ...
    const { error: upErr } = await supabaseAdmin.storage.from(bucket).upload(storagePath, pdfBuffer, { contentType: "application/pdf", upsert: true });
...
module.exports = { renderFront, makePdfBuffer, generateAndUploadCardPDF };
